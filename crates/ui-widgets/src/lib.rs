...
mod tests {
    ...
    fn button_click_emits_button_clicked() {
        ...
        let btn = tree
            .button("submit", "Submit", true, leaf_style(120.0, 40.0))
            .unwrap();
        ...
        assert_eq!(
            event,
            Some(ui_core::UiEvent::ButtonClicked {
                widget_id: "submit".to_string()
            })
        );
    }
    ...
    fn disabled_button_does_not_emit_event() {
        let mut tree = WidgetTree::new();
        let btn = tree
            .button("submit", "Submit", false, leaf_style(120.0, 40.0))
            .unwrap();
        let root = tree.container(&[btn], leaf_style(200.0, 200.0)).unwrap();
        tree.compute(root, Size::MAX_CONTENT).unwrap();
    ...
    fn checkbox_click_toggles_the_opposite_of_current_state() {
        let mut tree = WidgetTree::new();
        let cb = tree
            .checkbox("accept_terms", false, leaf_style(24.0, 24.0))
            .unwrap();
        ...
        assert_eq!(
            event,
            Some(ui_core::UiEvent::CheckboxToggled {
                widget_id: "accept_terms".to_string(),
                checked: true
            })
        );
    }
    ...
    fn tabbar_click_reports_owner_id_and_clicked_index() {
        ...
        assert_eq!(
            event,
            Some(ui_core::UiEvent::TabSelected {
                widget_id: "settings_tabs".to_string(),
                tab_index: 1
            })
        );
    }
    ...
    fn click_outside_any_widget_yields_none() {
        let mut tree = WidgetTree::new();
        let btn = tree
            .button("submit", "Submit", true, leaf_style(50.0, 20.0))
            .unwrap();
        let root = tree.container(&[btn], leaf_style(200.0, 200.0)).unwrap();
        tree.compute(root, Size::MAX_CONTENT).unwrap();
    ...
    fn build_frame_emits_one_instance_and_one_text_per_button() {
        ...
        let root = tree
            .container(&[btn, label], leaf_style(200.0, 200.0))
            .unwrap();
        tree.compute(root, Size::MAX_CONTENT).unwrap();

        let frame = tree
            .build_frame(root, &Theme::cyber_glass(), InteractionState::default())
            .unwrap();
        assert_eq!(
            frame.instances.len(),
            1,
            "only button produces SDF quad, container produces none"
        );
        assert_eq!(
            frame.texts.len(),
            2,
            "button and label each produce one text spec"
        );
    }
    ...
    fn corner_radius_never_exceeds_half_the_smaller_side() {
        ...
        assert!(
            ...
            "theme must exceed half of side to test clamping"
        );

        let cb = tree
            .checkbox("accept", true, leaf_style(24.0, 24.0))
            .unwrap();
        let root = tree.container(&[cb], leaf_style(200.0, 200.0)).unwrap();
        tree.compute(root, Size::MAX_CONTENT).unwrap();

        let frame = tree
            .build_frame(root, &theme, InteractionState::default())
            .unwrap();
        assert_eq!(frame.instances.len(), 1);
        assert!(
            frame.instances[0].radius <= 12.0,
            "radius must be clamped to half of side (12), got {}",
            frame.instances[0].radius
        );
    }
    ...
    fn hovering_a_button_boosts_its_glow_intensity() {
        ...
        let hovered_key = crate::InteractionKey {
            widget_id: key,
            index: None,
        };
        let hovered = tree
    ...
    fn window_close_button_dispatches_to_owner_window_id() {
        ...
        let window = tree
            .window("prefs_window", "Preferences", &[content], window_style)
            .unwrap();
        tree.compute(window, Size::MAX_CONTENT).unwrap();

        // Close button is anchored at top-right with 20x20 size.
        let event = tree
            .dispatch_click(window, (300.0 - 8.0 - 10.0, 8.0 + 10.0))
            .unwrap();
        assert_eq!(
            event,
            Some(ui_core::UiEvent::WindowCloseRequested {
                widget_id: "prefs_window".to_string()
            })
        );
    }
    ...
    fn effective_bounds_shift_children_by_scroll_offset_and_clip_to_viewport() {
        ...
        let mut tree = WidgetTree::new();
        let btn = tree
            .button("scrolled_btn", "Click", true, leaf_style(50.0, 50.0))
            .unwrap();
        let scroll = tree
            .scrollview("scroll_area", [0.0, 30.0], &[btn], leaf_style(100.0, 100.0))
            .unwrap();
    ...
    fn scrolled_button_is_clickable_at_its_new_position_and_not_at_its_old_one() {
        ...
        assert_eq!(
            hit,
            Some(ui_core::UiEvent::ButtonClicked {
                widget_id: "scrolled_btn".to_string()
            })
        );
    ...
    fn scrollview_does_not_shrink_content_to_fit_viewport() {
        ...
        let viewport_style = Style {
            ...
            ..leaf_style(100.0, 50.0)
        };
        let scroll = tree
            .scrollview("scroller", [0.0, 0.0], &[tall_child], viewport_style)
            .unwrap();
        tree.compute(scroll, Size::MAX_CONTENT).unwrap();

        let bounds = tree.resolved_bounds(scroll).unwrap();
        assert_eq!(
            bounds[&tall_child][3], 300.0,
            "content height must not be squashed to viewport"
        );
    }
    ...
    fn scroll_over_a_child_widget_still_scrolls_its_scrollview_ancestor() {
        ...
        let list = tree
            ...
            .unwrap();
        let scroll = tree
            .scrollview("scroll_area", [0.0, 0.0], &[list], leaf_style(100.0, 40.0))
            .unwrap();
        tree.compute(scroll, Size::MAX_CONTENT).unwrap();

        let event = tree
            .dispatch_scroll(scroll, (10.0, 10.0), [0.0, 15.0])
            .unwrap();
        assert_eq!(
            event,
            Some(ui_core::UiEvent::ScrollChanged {
                widget_id: "scroll_area".to_string(),
                offset: [0.0, 15.0]
            })
        );
    }
    ...
    fn toggle_click_emits_toggle_switched_with_opposite_state() {
        let mut tree = WidgetTree::new();
        let toggle = tree
            .toggle("turbo_mode", false, leaf_style(44.0, 24.0))
            .unwrap();
        ...
        assert_eq!(
            event,
            Some(ui_core::UiEvent::ToggleSwitched {
                widget_id: "turbo_mode".to_string(),
                active: true
            })
        );
    }
    ...
    fn slider_click_and_drag_calculates_interpolated_value() {
        let mut tree = WidgetTree::new();
        let slider = tree
            .slider("volume", 0.0, 100.0, 25.0, leaf_style(200.0, 20.0))
            .unwrap();
        ...
        assert_eq!(
            event,
            Some(ui_core::UiEvent::SliderChanged {
                widget_id: "volume".to_string(),
                value: 50.0
            })
        );
        ...
        let drag_val = tree
            .slider_drag_value(root, "volume", (180.0, 50.0))
            .unwrap();
    ...
    fn vertical_slider_click_and_drag_calculates_interpolated_value() {
        ...
        let slider = tree
            .slider_vertical("fader", 0.0, 100.0, 0.0, leaf_style(30.0, 200.0))
            .unwrap();
        ...
        let drag_val = tree
            .slider_drag_value(root, "fader", (100.0, 180.0))
            .unwrap()
            .unwrap();
    ...
    fn progress_indicators_generate_instances_and_labels() {
        ...
        let v_bar = tree
            .progress_bar_vertical(0.50, leaf_style(8.0, 100.0))
            .unwrap();
        let ring = tree
            .progress_ring(0.85, Some("85%"), leaf_style(60.0, 60.0))
            .unwrap();
        let pie = tree
            .progress_pie(0.40, Some("40%"), leaf_style(60.0, 60.0))
            .unwrap();
        let root = tree
            .container(&[h_bar, v_bar, ring, pie], leaf_style(400.0, 200.0))
            .unwrap();
        tree.compute(root, Size::MAX_CONTENT).unwrap();
        ...
        assert!(
            ...
            "all progress indicators produce GPU instances"
        );
        assert!(
            frame.texts.iter().any(|t| t.text == "85%"),
            "ring produces central label"
        );
        assert!(
            frame.texts.iter().any(|t| t.text == "40%"),
            "pie produces central label"
        );
    }
    ...
    fn text_input_click_emits_focus_changed() {
        ...
        assert_eq!(
            event,
            Some(ui_core::UiEvent::FocusChanged {
                widget_id: Some("search_box".to_string())
            })
        );
    }
    ...
    fn radio_click_emits_radio_selected() {
        ...
        assert_eq!(
            event,
            Some(ui_core::UiEvent::RadioSelected {
                group_id: "mode_group".to_string(),
                selected_id: "opt_a".to_string()
            })
        );
    }
    ...
    fn segmented_control_click_emits_segment_selected() {
        ...
        let seg = tree
            .segmented_control(
                ...
                &options,
                0,
                leaf_style(50.0, 28.0),
                leaf_style(150.0, 28.0),
            )
            .unwrap();
        ...
        assert_eq!(
            event,
            Some(ui_core::UiEvent::SegmentSelected {
                widget_id: "time_span".to_string(),
                selected_index: 1
            })
        );
    }
    ...
    fn modal_backdrop_click_emits_modal_dismissed() {
        let mut tree = WidgetTree::new();
        let inner = tree
            .label("Modal Message", leaf_style(100.0, 20.0))
            .unwrap();
        let modal = tree
            .modal(
                "confirm_dialog",
                "Confirmation",
                &[inner],
                leaf_style(200.0, 100.0),
                leaf_style(400.0, 400.0),
            )
            .unwrap();
        ...
        assert_eq!(
            event,
            Some(ui_core::UiEvent::ModalDismissed {
                modal_id: "confirm_dialog".to_string()
            })
        );
    }
    ...
    fn menubar_click_emits_menu_toggled() {
        ...
        let bar = tree
            .menubar(
                ...
                &menus,
                None,
                leaf_style(60.0, 24.0),
                leaf_style(200.0, 24.0),
            )
            .unwrap();
        tree.compute(bar, Size::MAX_CONTENT).unwrap();
        ...
        assert_eq!(
            event,
            Some(ui_core::UiEvent::MenuToggled {
                menu_id: "main_menu:0".to_string(),
                open: true
            })
        );
    }
    ...
    fn menu_popover_click_emits_menu_item_clicked() {
        ...
        let popover = tree
            .menu_popover(
                "file_menu",
                &items,
                leaf_style(160.0, 28.0),
                leaf_style(160.0, 84.0),
            )
            .unwrap();
        ...
        assert_eq!(
            event1,
            Some(ui_core::UiEvent::MenuItemClicked {
                menu_id: "file_menu".to_string(),
                item_id: "item_new".to_string()
            })
        );
    ...
    fn grid_layout_distributes_children_into_columns() {
        let mut tree = WidgetTree::new();
        let b1 = tree
            .button("b1", "1", true, leaf_style(100.0, 30.0))
            .unwrap();
        let b2 = tree
            .button("b2", "2", true, leaf_style(100.0, 30.0))
            .unwrap();
        let b3 = tree
            .button("b3", "3", true, leaf_style(100.0, 30.0))
            .unwrap();
        let b4 = tree
            .button("b4", "4", true, leaf_style(100.0, 30.0))
            .unwrap();
    ...
    fn tree_view_dispatches_toggle_and_selection() {
        ...
        assert_eq!(
            ev1,
            Some(ui_core::UiEvent::TreeNodeToggled {
                tree_id: "proj_tree".to_string(),
                node_id: "src_dir".to_string(),
                expanded: false
            })
        );
        ...
        assert_eq!(
            ev2,
            Some(ui_core::UiEvent::TreeNodeSelected {
                tree_id: "proj_tree".to_string(),
                node_id: "main_rs".to_string()
            })
        );
    }
    ...
    fn split_view_calculates_drag_ratio() {
        let mut tree = WidgetTree::new();
        let p1 = tree
            .button("p1", "Panel 1", true, leaf_style(100.0, 100.0))
            .unwrap();
        let p2 = tree
            .button("p2", "Panel 2", true, leaf_style(100.0, 100.0))
            .unwrap();
        let split = tree
            .split_view(
                "main_split",
        ...
        let ratio = tree
            .split_ratio_at(split, "main_split", (60.0, 50.0))
            .unwrap();
    ...
    fn text_area_focus_and_multiline_frame() {
        ...
        assert_eq!(
            ev,
            Some(ui_core::UiEvent::FocusChanged {
                widget_id: Some("code_editor".to_string())
            })
        );
    ...
    fn password_input_reveal_toggle() {
        ...
        assert_eq!(
            ev,
            Some(ui_core::UiEvent::PasswordRevealed {
                widget_id: "user_pwd".to_string(),
                revealed: true
            })
        );
    ...
    fn number_input_stepper_and_clamping() {
        ...
        assert_eq!(
            ev_inc,
            Some(ui_core::UiEvent::NumberChanged {
                widget_id: "spin_count".to_string(),
                value: 15.0
            })
        );
    ...
    fn keyboard_tab_focus_cycle() {
        let mut tree = WidgetTree::new();
        let i1 = tree
            .text_input("input_1", "", "Input 1", false, leaf_style(100.0, 30.0))
            .unwrap();
        let i2 = tree
            .password_input(
                "input_pwd",
                ...
                false,
                false,
                leaf_style(100.0, 30.0),
            )
            .unwrap();
        let b1 = tree
            .button("btn_submit", "Submit", true, leaf_style(100.0, 30.0))
            .unwrap();
        let form = tree
            .container(&[i1, i2, b1], leaf_style(300.0, 100.0))
            .unwrap();
        tree.compute(form, Size::MAX_CONTENT).unwrap();

        // Initial focus (None) forward -> first focusable ("input_1")
        assert_eq!(
            tree.next_focusable(form, None, false).unwrap(),
            Some("input_1".to_string())
        );

        // Tab forward from "input_1" -> "input_pwd"
        assert_eq!(
            tree.next_focusable(form, Some("input_1"), false).unwrap(),
            Some("input_pwd".to_string())
        );

        // Tab forward from "input_pwd" -> "btn_submit"
        assert_eq!(
            tree.next_focusable(form, Some("input_pwd"), false).unwrap(),
            Some("btn_submit".to_string())
        );

        // Tab forward from "btn_submit" -> wraps around to "input_1"
        assert_eq!(
            tree.next_focusable(form, Some("btn_submit"), false)
                .unwrap(),
            Some("input_1".to_string())
        );
        ...
        assert_eq!(
            tree.next_focusable(form, Some("input_1"), true).unwrap(),
            Some("btn_submit".to_string())
        );
    }
    ...
    fn icon_and_icon_button_render_and_click() {
        ...
        let icon_node = tree
            .icon(
                crate::kind::IconKind::Shield,
                16.0,
                None,
                leaf_style(24.0, 24.0),
            )
            .unwrap();
        let btn_node = tree
            .icon_button(
                "gear_btn",
                crate::kind::IconKind::Settings,
                true,
                leaf_style(32.0, 32.0),
            )
            .unwrap();
        let cont = tree
            .container(&[icon_node, btn_node], leaf_style(100.0, 40.0))
            .unwrap();
    ...
    fn table_header_click_and_row_selection() {
        ...
        assert_eq!(
            ev_hdr,
            Some(ui_core::UiEvent::TableHeaderClicked {
                table_id: "mesh_table".to_string(),
                column_index: 0
            })
        );
        ...
        assert_eq!(
            ev_row,
            Some(ui_core::UiEvent::TableRowSelected {
                table_id: "mesh_table".to_string(),
                row_index: 1
            })
        );
    ...
    fn accordion_toggle_and_children_collapse() {
        let mut tree = WidgetTree::new();
        let body = tree
            .button("acc_btn", "Inner Action", true, leaf_style(100.0, 30.0))
            .unwrap();
        let acc_collapsed = tree
            .accordion(
                "acc_sec",
                "Diagnostics",
                Some("Real-time telemetry"),
                false,
                body,
                leaf_style(200.0, 40.0),
            )
            .unwrap();
        ...
        assert_eq!(
            ev,
            Some(ui_core::UiEvent::AccordionToggled {
                id: "acc_sec".to_string(),
                expanded: true
            })
        );
    ...
    fn breadcrumb_and_pagination_events() {
        ...
        let bc = tree
            .breadcrumb(
                "main_bc",
                &crumbs,
                leaf_style(60.0, 20.0),
                leaf_style(200.0, 20.0),
            )
            .unwrap();
        ...
        assert_eq!(
            ev,
            Some(ui_core::UiEvent::BreadcrumbClicked {
                bar_id: "main_bc".to_string(),
                index: 0,
        ...
        let pag = tree
            .pagination(
                ...
                2,
                5,
                leaf_style(30.0, 24.0),
                leaf_style(240.0, 24.0),
            )
            .unwrap();
        ...
        assert_eq!(
            ev_next,
            Some(ui_core::UiEvent::PageSelected {
                widget_id: "data_pag".to_string(),
                page: 3
            })
        );
    }
    ...
    fn badge_and_color_swatch_render() {
        ...
        assert_eq!(
            ev,
            Some(ui_core::UiEvent::ColorSelected {
                widget_id: "theme_accent".to_string(),
    ...
    fn color_picker_renders_and_dispatches_mode_switches() {
        ...
        assert!(matches!(
            ev_hue,
            Some(ui_core::UiEvent::ColorChanged { .. })
        ));
        ...
        assert_eq!(
            ev_card,
            Some(ui_core::UiEvent::SelectChanged {
    ...
    fn palette_folding_closing_and_dragging() {
        let mut tree = WidgetTree::new();
        let btn = tree
            .button("brush_btn", "Brush", true, leaf_style(80.0, 26.0))
            .unwrap();
        let pal = tree
            .palette(
                "test_palette",
        ...
        assert_eq!(
            ev_fold,
            Some(ui_core::UiEvent::PaletteFoldToggled {
        ...
        assert_eq!(
            ev_close,
            Some(ui_core::UiEvent::PaletteClosed {
                palette_id: "test_palette".to_string(),
            })
        );
    }
    ...
    fn text_cursor_click_and_caret_positioning() {
        ...
        let cur_start = tree
            .text_cursor_at(root, (12.0, 16.0), &theme.typography.family, 15.0, None)
            .unwrap();
        assert_eq!(cur_start, Some(("search".to_string(), 0)));

        // Click far to the right -> char index 6 (end of string)
        let cur_end = tree
            .text_cursor_at(root, (180.0, 16.0), &theme.typography.family, 15.0, None)
            .unwrap();
    ...
    fn textarea_multiline_cursor_and_selection() {
        ...
        let cur_line2 = tree
            .text_cursor_at(root, (50.0, 32.0), &theme.typography.family, 15.0, None)
            .unwrap();
    ...
    fn audio_visualizer_generates_proportional_frequency_bars() {
        ...
        let viz = tree
            .audio_visualizer("spectrum_bars", &spectrum, 1.0, leaf_style(200.0, 60.0))
            .unwrap();
    ...
    fn custom_paint_renders_shapes_and_dispatches_events() {
        ...
        painter
            .line([0.0, 0.0], [50.0, 50.0], 2.0, [1.0, 1.0, 0.0, 1.0])
            .circle([80.0, 80.0], 20.0, Some([0.0, 1.0, 0.5, 0.8]), None)
            .rect(
                [10.0, 10.0, 60.0, 40.0],
                4.0,
                Some([0.2, 0.2, 0.4, 0.5]),
                Some(([1.0, 0.0, 0.5, 1.0], 1.5)),
            )
            .bezier(
                [0.0, 100.0],
                [30.0, 50.0],
            ...
            .polyline(
                vec![[0.0, 0.0], [20.0, 10.0], [40.0, 0.0]],
                2.0,
                [1.0, 0.5, 0.0, 1.0],
                false,
            )
